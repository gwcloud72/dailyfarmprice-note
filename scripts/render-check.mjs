...
const expectedText = {"home": "오늘 가격 흐름을 빠르게 확인하세요", "items": "품목별 가격 정보", "markets": "시장별 가격 정보", "trend": "가격 동향 분석", "regions": "지역별 비교", "stats": "통계 정보", "market-news": "시장 동향", "alerts": "알림 서비스", "download": "데이터 받기", "favorites": "즐겨찾는 품목", "guide": "이용 안내"};
const errors = [];
global.window = { location: { hash: '' }, history: { replaceState(_state, _title, url) { global.window.location.hash = String(url || '').replace(/^[^#]*/, ''); } }, addEventListener() {}, removeEventListener() {}, setTimeout(callback) { callback(); return 0; }, clearTimeout() {} };
