function DocumentIcon() {
  return (
    <svg viewBox="0 0 48 48" className="mx-auto mb-3 size-12 text-emerald-300" aria-hidden="true">
      <path d="M13 7h17l8 8v26H13z" fill="none" stroke="currentColor" strokeWidth="3" strokeLinejoin="round" />
      <path d="M29 7v9h9M18 24h18M18 31h14" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" />
    </svg>
  );
}
...
function EmptyReport({ onReset }) {
  return (
    <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-center text-sm font-bold text-slate-500">
      ...
      {onReset && <button type="button" onClick={onReset} aria-label="리포트 조건 초기화" className="mt-4 rounded-xl bg-emerald-700 px-4 py-2 text-xs font-extrabold text-white shadow-sm shadow-emerald-900/10">조건 초기화</button>}
    </div>
  );
}
  ...
  return (
    <section className={`rounded-2xl border border-slate-200 bg-white shadow-sm ${isDocument ? 'p-6 md:p-7' : compact ? 'p-4' : 'p-5'}`}>
      <div className={isDocument ? 'mx-auto max-w-[820px]' : ''}>
        <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-emerald-700">{eyebrow}</p>
        <h2 className={`${isDocument ? 'mt-2 text-2xl' : 'mt-1 text-lg'} font-black text-slate-950`}>{title}</h2>
